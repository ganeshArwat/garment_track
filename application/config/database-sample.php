...
if ($segment1 == 'api_mobile' && isset($post_data['company_id'])) {
  ...
  if (isset($all_header['authorization']) && $all_header['authorization'] != '') {
    $auth_token = str_replace("Bearer", "", $all_header['authorization']);
  } else if (isset($all_header['authorization']) && $all_header['authorization'] != '') {
    $auth_token = str_replace("Bearer", "", $all_header['authorization']);
  }
  ...
  if (isset($auth_token)) {
    $decode_token =  base64_decode($auth_token);

    $decode_token_arr = explode("_", $decode_token);
    $company_id = isset($decode_token_arr[1]) ? $decode_token_arr[1] : '';
  }
  ...
  file_put_contents(FCPATH . '/log1/api_db.txt', serialize($database_name));
}
$db['default'] = array(
  'dsn' => '',
  'hostname' => 'localhost',
