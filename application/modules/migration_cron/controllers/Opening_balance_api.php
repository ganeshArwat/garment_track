<?php
class Opening_balance_api extends MX_Controller
{
    ...
    public function get_ids()
    {
        ...
        if ($base_url != '') {
            $url = $base_url . "api/v5/opening_balances/get_opening_balance_ids";
            ...
            if (isset($result['data']) && is_array($result['data']) && count($result['data']) > 0) {
                foreach ($result['data'] as $rkey => $rvalue) {
                    $qry = "SELECT id FROM opening_balance WHERE status IN(1,2) AND migration_id='" . $rvalue . "'";
                ...
                if (isset($insert_data) && is_array($insert_data) && count($insert_data) > 0) {
                    $this->db->insert_batch('opening_balance', $insert_data);
    ...
    public function get_api_data($company_name = '')
    {
        ...
        if ($base_url != '') {
            $api_url = $base_url . "api/v5/opening_balances/fetch_opening_balances";

            $qry = "SELECT  id,config_value FROM migration_log WHERE status IN(1,2) AND config_key='opening_balance_migrate_last_id' ";
            $qry_exe = $this->db->query($qry);
            ...
            $qry = "SELECT migration_id FROM opening_balance WHERE migration_id > " . $last_id . " ORDER BY migration_id  LIMIT " . $limit . "";
            $qry_exe = $this->db->query($qry);
        ...
        if (isset($result['data']) && is_array($result['data']) && count($result['data']) > 0) {
            ...
            foreach ($result['data'] as $key => $value) {
                ...
                if ($value['payment_type'] != '') {
                    $payment_type_name = strtolower($value['payment_type']);
                    if ($payment_type_name == 'credit') {
                        $payment_type = 1;
                    } else if ($payment_type_name == 'debit') {
                        $payment_type = 2;
                    }
                }

                $payment_insert['opening'] = array(
                    'customer_id' => $customer_id,
                    'payment_type' => $payment_type,
                    'opening_amount' =>  $value['amount'],
                    'leftover_amt' =>  $value['amount'],
                    'opening_date' =>  $value['opening_balance_date'],
                ...
                $qry = "SELECT id FROM opening_balance WHERE status IN(1,2) AND migration_id='" . $value['id'] . "'";
                $qry_exe = $this->db->query($qry);
                $existData = $qry_exe->row_array();


                if (isset($existData) && is_array($existData) && count($existData) > 0) {
                    $payment_insert['opening_id'] = $existData['id'];
                    $payment_id = $this->opening_balance->update($payment_insert);
                } else {
                    $payment_id = $this->opening_balance->insert($payment_insert);
                }

                $updateq = "UPDATE docket_include_data SET status=3,modified_date='3000-00-00 00:00:00' WHERE credit_id= '" . $payment_id . "' AND credit_id_type=3";
            ...
            WHERE i.status IN(1,2) AND i.invoice_id ='" . $invoice_id . "' AND i.invoice_type=1";
                ...
                if (isset($configExist) && is_array($configExist) && count($configExist) > 0) {
                    $updateq = "UPDATE migration_log SET config_value='" . $value['id'] . "' WHERE status IN(1,2) AND config_key='opening_balance_migrate_last_id'";
                    $this->db->query($updateq);
                } else {
                    $mig_insert_data = array(
                        'config_key' => 'opening_balance_migrate_last_id',
                        'config_value' => $value['id']
                    );
                    $this->gm->insert('migration_log', $mig_insert_data);
                }
                echo "<br>OPENING ID= " . $value['id'] . " added";
            }
        }
