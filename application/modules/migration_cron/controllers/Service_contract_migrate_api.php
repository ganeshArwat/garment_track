<?php
class Service_contract_migrate_api extends MX_Controller
{
    ...
    public function get_ids()
    {
        ...
        if ($base_url != '') {
            $url = $base_url . "api/v5/vendor_contracts/get_vendor_contract_ids";
            ...
            if (isset($result['data']) && is_array($result['data']) && count($result['data']) > 0) {
                foreach ($result['data'] as $rkey => $rvalue) {
                    $qry = "SELECT id FROM vendor_contract WHERE status IN(1,2) AND migration_id='" . $rvalue . "'";
                ...
                if (isset($insert_data) && is_array($insert_data) && count($insert_data) > 0) {
                    $this->db->insert_batch('vendor_contract', $insert_data);
    ...
    public function get_api_data($company_name = '')
    {
        ...
        if ($base_url != '') {
            $api_url = $base_url . "api/v5/vendor_contracts/fetch_vendor_contracts";


            $qry = "SELECT  id,config_value FROM migration_log WHERE status IN(1,2) AND config_key='service_contract_migrate_last_id' ";
            $qry_exe = $this->db->query($qry);
            ...
            $qry = "SELECT migration_id FROM vendor_contract WHERE migration_id > " . $last_id . " ORDER BY migration_id  LIMIT " . $limit . "";
            $qry_exe = $this->db->query($qry);
        ...
        if (isset($result) && is_array($result) && count($result) > 0) {
            ...
            $this->load->module('service_contracts');
            ...
            if (isset($result['data']) && is_array($result['data']) && count($result['data']) > 0) {
                ...
                foreach ($result['data'] as $key => $value) {
                    ...
                    $qry = "SELECT id FROM vendor_contract WHERE status IN(1,2) AND migration_id='" . $value['id'] . "'";
                    $qry_exe = $this->db->query($qry);
                    $existData = $qry_exe->row_array();

                    $old_rule = array();
                    if (isset($existData) && is_array($existData) && count($existData) > 0) {
                        $qry = "SELECT id,migration_id FROM vendor_contract_rate WHERE status IN(1,2) AND vendor_contract_id='" . $existData['id'] . "'";
                        $qry_exe = $this->db->query($qry);
                    ...
                    if (isset($value['vendor_contract_rules']) && is_array($value['vendor_contract_rules']) && count($value['vendor_contract_rules']) > 0) {
                        foreach ($value['vendor_contract_rules'] as $ckey => $cvalue) {
                    ...
                    if (isset($existData) && is_array($existData) && count($existData) > 0) {
                        ...
                        $this->service_contracts->update($customer_insert);
                    } else {
                        $this->service_contracts->insert($customer_insert);
                    }
                    ...
                    if (isset($configExist) && is_array($configExist) && count($configExist) > 0) {
                        $updateq = "UPDATE migration_log SET config_value='" . $value['id'] . "' WHERE status IN(1,2) AND config_key='service_contract_migrate_last_id'";
                        $this->db->query($updateq);
                    } else {
                        $mig_insert_data = array(
                            'config_key' => 'service_contract_migrate_last_id',
                            'config_value' => $value['id']
                        );
                        $this->gm->insert('migration_log', $mig_insert_data);
                    }
                    echo "<br>SERVICE CONTRACT added";
                }
            }
        }
