...
class Custom_validation extends MX_Controller
{
    public function __construct()
    {
        ...
        $this->heading = 'Custom Validation';
    }
    ...
    public function show_form($module_id = 0)
    {
        ...
        $edit_data = $this->gm->get_data_list('custom_validation_field', array('module_id' => $module_id, 'status' => 1, 'validation_user' => 1), array(), array(), 'id,label_key,validation_type');
        ...
        if ($data['setting_data']['docket_col_sort'] == '') {
            $data['setting_data']['docket_col_sort'] = 'awb_no,booking_date,forwarding_no,customer_id,product_name,vendor_name,co_vendor_name,origin_name,destination_name,consignee_name,shipper_name';
        }
    ...
    public function insert_data()
    {
        ...
        if (isset($post) && is_array($post) && count($post) > 0) {
            ...
            $summary_show = $this->input->post('summary_show');

            $deleq = "DELETE FROM custom_validation_field WHERE validation_user=1 AND module_id=" . $post['module_id'];
            $this->db->query($deleq);
            ...
            if (isset($summary_show) && is_array($summary_show) && count($summary_show) > 0) {
                foreach ($summary_show as $skey => $svalue) {
                    $setting_insert = array(
                        'module_id' => $post['module_id'],
                        'label_key' => $svalue,
                        'validation_type' => 4,
            ...
            $qry = "UPDATE setting_data SET config_value='' WHERE status=1 AND config_key='kyc_setting'";
            $this->db->query($qry);
            ...
            if (isset($post['kyc_field']) && is_array($post['kyc_field']) && count($post['kyc_field']) > 0) {
                ...
                $setting_insert = array(
                    ...
                    'config_key' => 'kyc_setting',
                    'created_date' => date('Y-m-d H:i:s'),
                    'created_by' => $this->user_id
                );
                $qry = "SELECT id FROM setting_data WHERE status=1 AND config_key='kyc_setting'";
        ...
        redirect(site_url('settings/custom_validation/show_form/' . $post['module_id']));
    }
}
