<?php
class Service_contract extends MX_Controller
{
            ...
            if (isset($csvData) && is_array($csvData) && count($csvData) > 0) {
                ...
                foreach ($csvData as $u_key => $cvalue) {
                    ...
                    if (!isset($cvalue['vendor_code']) || $cvalue['vendor_code'] == '') {
                        $data['error'] = 'SERVICE NOT FOUND on row ' . $count . '';
                        ...
                        $recordExist = array();
                        if ((int)$cvalue['id'] > 0) {
                            $recordExist = $this->gm->get_selected_record('vendor_contract', 'id', array('migration_id' => $cvalue['id']), array());
                        }
                        if (isset($recordExist) && is_array($recordExist) && count($recordExist) > 0) {
                            $this->gm->update('vendor_contract', $insert_data, '', array('id' => $recordExist['id']));
                            $contract_id = $recordExist['id'];
                            $data['error'] = 'CONTRACT UPDATED SUCCESSFULLY on row ' . $count . '';
                            $non_inserted_data[] = $data;
                        } else {
                            $contract_id = $this->gm->insert('vendor_contract', $insert_data);
                        }
                        ...
                        if (isset($onadd_arr) && is_array($onadd_arr) && count($onadd_arr) > 0) {
                            $deleteq = "DELETE FROM vendor_contract_rate WHERE vendor_contract_id='" . $contract_id . "'";
                            ...
                            foreach ($onadd_arr as $rate_key => $rate_value) {
                                ...
                                $this->gm->insert('vendor_contract_rate', $rate_insert);
                            }
                        }
                ...
                if (isset($non_inserted_data) && is_array($non_inserted_data) && count($non_inserted_data) > 0) {
                    ...
                    $this->session->set_flashdata('add_feedback', 'SERVICE Contract imported successfully.');
                    redirect(site_url('service_contracts/show_list'));
                }
            } else {
                $this->session->set_flashdata('add_feedback', 'No Data present in file');
                redirect(site_url('mirgation/import_data/show_form/service_contract'));
            }
        } else {
